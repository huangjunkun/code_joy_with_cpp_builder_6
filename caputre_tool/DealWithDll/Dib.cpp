 ...
 * 函数功能: 获取位图数据
*******************************************************************************/
bool CDib::GetColorref(HBITMAP m_bitmap)
{
    if(m_bitmap==NULL)
    {
        return FALSE;
...
extern HBITMAP Zoom(HBITMAP bitmap,int lDstWidth,int lDstHeight);
void CDib::Initnewpic()
{
    int wider,height;
    if(bm.bmWidth>bm.bmHeight)
    {
        wider=100;
        height=(int)(100*((float)bm.bmHeight)/bm.bmWidth);
    }
    else
    {
        height=100;
        wider=(int)(100*((float)bm.bmWidth)/bm.bmHeight);
    }
    newbmp=Zoom(m_hbmp,wider,height);
    if(newbmp!=NULL)
    {
        HDRAWDIB hdraw;
//    	HBITMAP srcbmp1;
        ...
        bmp.Attach(newbmp);
        bmp.GetBitmap(&newbm);
        NEWRGB32BITMAPINFO.biSize=sizeof(BITMAPINFOHEADER);
        NEWRGB32BITMAPINFO.biWidth=newbm.bmWidth;
        NEWRGB32BITMAPINFO.biHeight=newbm.bmHeight;
        NEWRGB32BITMAPINFO.biPlanes=1;
        NEWRGB32BITMAPINFO.biBitCount=newbm.bmPlanes*newbm.bmBitsPixel;
        NEWRGB32BITMAPINFO.biCompression=BI_RGB;
        NEWRGB32BITMAPINFO.biSizeImage=0;
        NEWRGB32BITMAPINFO.biXPelsPerMeter=0;
        NEWRGB32BITMAPINFO.biYPelsPerMeter=0;
        NEWRGB32BITMAPINFO.biClrUsed=0;
        NEWRGB32BITMAPINFO.biClrImportant=0;
        srcbmp1=CreateDIBSection(memdc,(BITMAPINFO*)&NEWRGB32BITMAPINFO,
                                 DIB_RGB_COLORS,(VOID**)&newpcol,NULL,0);
        if(srcbmp1)
        {
            HBITMAP hOldBmp=(HBITMAP)SelectObject(memdc,srcbmp1);
            HDC hDC=CreateCompatibleDC(memdc);
            if(hDC)
            {
                HBITMAP hOldBmp2=(HBITMAP)SelectObject(hDC,bmp);
                //将DDB的数据复制到DIB中
                BitBlt(memdc,0,0,newbm.bmWidth,newbm.bmHeight,hDC,
                       0,0,SRCCOPY);
                SelectObject(hDC,hOldBmp2);
                DeleteDC(hDC);
            }
